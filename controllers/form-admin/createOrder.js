...
exports.createOrder = async (req, res, next) => {
    ...
    body.product.map((item, i) => {
        rowData.push({
            ...
            "จ่ายที่เหลือภายในวันที่": (item.paid_date === 'Invalid date' || item.paid_date === '') ? '' : moment(item.paid_date).format('M/D/YYYY'),
            "เว็บจัดส่งภายในวันที่": (item.release_date === 'Invalid date' || item.release_date === '') ? '' : moment(item.release_date).format('M/D/YYYY'),
